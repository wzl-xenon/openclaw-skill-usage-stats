...
def generate_custom_chart(start_time, end_time):
    print(f"📊 正在生成自定义时间范围统计：{start_time} 至 {end_time}")
    # 调用统计脚本获取数据
    result = subprocess.check_output([
        'python3', 
        '/root/.openclaw/workspace/skills/openclaw-usage-stats/scripts/get_usage_stats.py', 
        start_time, end_time,
        '--force' # 强制刷新最新数据
    ]).decode('utf-8')
    lines = result.split('\n')
    ...
    values = [total_msg, user_msg, assistant_msg, tool_response_msg]
    # 画堆叠柱状图
    ...
    # 保存图片
    output_file = f'/root/.openclaw/workspace/usage_stats_custom_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
    plt.savefig(output_file, dpi=100, bbox_inches='tight')
    plt.close()
    
    print(f"✅ 自定义时间范围统计图像已生成：{output_file}")
    print("\n" + result)
    return output_file

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("⚠️  请输入自定义时间范围，格式示例：")
        print("   python generate_custom_usage_chart.py \"2026-04-01 09:00\" \"2026-04-01 12:00\"")
        print("   python generate_custom_usage_chart.py 2026-03-01 2026-03-31")
        sys.exit(1)
    
    start_time = sys.argv[1]
    end_time = sys.argv[2]
    generate_custom_chart(start_time, end_time)
